...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if([conn openSocket]){
            ...
            [conn writeData:mes protocol:4004];
...
{
    static NSString *CellIdentifier = @"Complete";
    ...
    // Configure the cell...
    if (_result) {
        NSArray *a = [_nodeNum objectAtIndex:indexPath.row];
        if ([a[0] isEqualToString:@""]) {
            cell.textLabel.text = @"Empty Set";
        }else{
            cell.textLabel.text = [NSString stringWithFormat:@"Complete %d",indexPath.row+1];
        }
    }else if(_complete){
